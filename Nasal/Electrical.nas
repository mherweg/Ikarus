...
####    Modified for C42 Ikarus by Ron Jensen  ####
####    Syd Adams    ####
...
var PDIMMER = props.globals.getNode("controls/lighting/panel-norm",1);
Battery={};
Alternator={};
...
#var battery = Battery.new(volts,amps,amp_hours,charge_percent,charge_amps);
var battery = Battery.new(23,20,10,1.0,7.0);
# var alternator = Alternator.new("rpm-source",rpm_threshold,volts,amps);
var alternator1 = Alternator.new("/engines/engine[0]/thruster/rpm",800.0,26.0,40.0);
...
setlistener("/sim/signals/fdm-initialized", func {
    ...
    props.globals.getNode("controls/lighting/dome-lights",1).setBoolValue(0);
    ...
    props.globals.getNode("/sim/signals/electrical-initialized",1).setBoolValue(1);
...
var update_virtual_bus = func( dt ) {
   ...
   if ( ALT.getBoolValue() and (alternator1_volts > bus_volts) ) {
    ...
    var avionics_switch = props.globals.getNode("controls/avionics/power", 1).getBoolValue();
    if (avionics_switch){
...
var electrical_bus = func() {
    bus_volts = arg[0];
    load = 0.0;
    var starter_switch = props.globals.getNode("controls/engines/engine[0]/starter-switch").getBoolValue();
    starter_volts = 0.0;
    if ( starter_switch) {
    ...
    OutPuts.getNode("landing-lights-norm",1).setValue(bus_volts * NORM);
        load += 1.0;
        } else {
    OutPuts.getNode("landing-lights-norm",1).setValue(0.0);
        }
        
    if ( props.globals.getNode("controls/lighting/dome-lights").getBoolValue()){
    OutPuts.getNode("dome-lights-norm",1).setValue(bus_volts * NORM);
        load += 0.5;
        } else {
    OutPuts.getNode("dome-lights-norm",1).setValue(0.0);
        }

    if ( props.globals.getNode("controls/lighting/taxi-light").getBoolValue()){
    OutPuts.getNode("taxi-lights-norm",1).setValue(bus_volts * NORM);
        load += 1.0;
        } else {
    OutPuts.getNode("taxi-lights-norm",1).setValue(0.0);
        }
    ...
    if ( props.globals.getNode("controls/engines/engine[0]/master-caution").getBoolValue()){
        OutPuts.getNode("master-caution-norm",1).setValue(bus_volts * NORM);
        load += bus_volts * NORM * 0.5;
        } else {
        OutPuts.getNode("master-caution-norm",1).setValue(0.0);
        }        
    OutPuts.getNode("DG",1).setValue(bus_volts);
    OutPuts.getNode("oat",1).setValue(bus_volts);
    OutPuts.getNode("turn-coordinator",1).setValue(bus_volts);
    OutPuts.getNode("attitude-indicator",1).setValue(bus_volts);
